...
function getTime(date) {
  ...
  return `${currDate.getHours().toString().padStart(2, '0')}:${currDate.getMinutes().toString().padStart(2, '0')}:${currDate.getSeconds().toString().padStart(2, '0')}`;
}
...
function getNextFriday(date) {
  const currDate = new Date(date);
  const dayToFriday = currDate.getDay() - 5;
  if (dayToFriday < 0) {
    currDate.setDate(currDate.getDate() - dayToFriday);
  } else {
    currDate.setDate(currDate.getDate() + 7 - dayToFriday);
  }
  return currDate;
}
...
function isDateInPeriod(date, period) {
  ...
  return (
    new Date(period.start) <= checkDate && checkDate <= new Date(period.end)
  );
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  function addDays(numberOfdays, dayOff) {
    let i = 0;
    while (currDay <= endDay && i < numberOfdays) {
      if (!dayOff) {
        arrSchedule.push(
          `${currDay.getDate().toString().padStart(2, '0')}-${(currDay.getMonth() + 1).toString().padStart(2, '0')}-${currDay.getFullYear()}`
        );
      }
      i += 1;
  ...
  while (currDay <= endDay) {
    addDays(countWorkDays, false);
    addDays(countOffDays, true);
  }
